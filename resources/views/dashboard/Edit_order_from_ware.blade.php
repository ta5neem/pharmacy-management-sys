                    ...
                    <form id="form" action="/orders/store" method="POST">
                      {{ csrf_field() }}
                        <div class="form-row">
                            ...
                            <input type="text" name="user_id" value="{{\App\User::select('name')->where('id',$user)->value('name')}}" class="form-control2  col-lg-3 readonly" id="user_id"
                                    ...
                                    <tr class="cloning-row" id="0">
                                        <td>
                                            ...
                                            @foreach ($medicine as $one)
                                                <option value="{{$one->generic_name}}">{{$one->generic_name}}</option>
                                                @endforeach
                                                @foreach ($MedicalSupply as $one)
                                                    <option value="{{$one->name}}">{{$one->name}}</option>
                                                    @endforeach
                                                    @foreach ($MedicalFood as $one)
                                                        <option value="{{$one->name}}">{{$one->name}}</option>
                                                        @endforeach
                        ...
                        <button type="submit" class="btn btn-list float-right add">Edit</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-add', function () {
                ...
                $('#invoice_details').find('tbody').append($('' +
                    ...
                    '<td><select class="selectpicker product_name" data-live-search="true" style="width: 120%;"name="product_id[' + numberIncr + ']" required="required"><option value=""></option><option value="c1">1</option><option value="f2">f2</option></select></td>'+
