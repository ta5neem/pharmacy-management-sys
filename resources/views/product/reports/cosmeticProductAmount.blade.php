        ...
        <div class="card">
            ...
            <div class="card-body">
                        ...
                        <div class="col-lg-5 col-md-12">
                            <div class="input-group ">
                                <div class="input-group-prepend">
                                    <span class="input-group-text total-span" id="basic-addon1">Total number of cosmetics:</span>
                                </div>
                                <input class="form-control read col-md-2" readonly value="{{$cosmeticProducts->count()}}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <table id="table1" class="table table-striped table-bordered dt-responsive nowrap"
                    ...
                    <tbody>
                    @foreach($cosmeticProducts as $id => $cosmeticProduct)
                        <tr>
                            <th scope="row">{{$id}}</th>
                            @foreach($cosmeticProduct as $one)
                                <td>{{$one->name}}</td>
                                <td>{{$cosmeticProduct ->count()}}</td>
                                @break
                            @endforeach
                            <td>{{$cosmeticProduct->sum('amount')}}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
